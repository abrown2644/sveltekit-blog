...
export async function POST({ request, locals }) {
	...
	const uploadedFiles = { images: [], videos: [] };
	...
	const uploadPromises = files.map(async (file) => {
		...
		switch (folder) {
			case 'videos':
				uploadedFiles.videos.push({ name: file.name, url: downloadURLs[0] });
				break;
			default:
				uploadedFiles.images.push({ name: file.name, url: downloadURLs[0] });
				break;
		}
	});
